...
class RobotLeg:
    ...
    def inverseKin1D(self, z: float) -> List[float]:
        try:
            ...
            cos_beta = (z**2 + self.upper_leg_length**2 - self.lower_leg_length**2) / (2 * z * self.upper_leg_length)
    ...
    def inverseKin2D(self, x: float, z: float) -> List[float]:
        try:
    ...
    def inverseKin3D(self, x: float, y: float, z: float) -> List[float]:
        try:
            ...
            gamma = math.atan2(y - self.hip_to_shoulder, z)
    ...
    def move_leg(self, x: float, y: float, z: float):
        try:
            ...
            servo_control.servo_move(self.servo_channels[0], gamma)
            servo_control.servo_move(self.servo_channels[1], beta)
            servo_control.servo_move(self.servo_channels[2], math.pi - alpha)
